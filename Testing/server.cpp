...
public:
	...
	void getfile(string filename , int sockfd)
	{
		ofstream f;
		...
		while(1)
		{
		    ...
		    int n = read(newsockfd,buffer,256);
     		...
     		{ 
   			...
   			/// Modularity Modificatoins :
   			/*
						    int n = read(newsockfd,buffer,256);
						    if(buffer=="file")
						    {
								Give the client a message to send the file 
								Also the client needs to send the name of the file first.

								Call the getfile function till the special string EOF is not encountered (The client will send it when the input file is terminated.)
